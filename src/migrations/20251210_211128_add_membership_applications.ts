...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`membership_applications\` (
  	...
  	\`hiv_experience\` text NOT NULL,
  	\`agreed_to_commitments\` integer DEFAULT false NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`membership_applications_updated_at_idx\` ON \`membership_applications\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`membership_applications_created_at_idx\` ON \`membership_applications\` (\`created_at\`);`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`membership_applications_id\` integer REFERENCES membership_applications(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`membership_applications\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "providers_id", "resources_id", "blog_id", "pdf_library_id", "tags_id", "faqs_id", "pages_id", "media_id") SELECT "id", "order", "parent_id", "path", "users_id", "providers_id", "resources_id", "blog_id", "pdf_library_id", "tags_id", "faqs_id", "pages_id", "media_id" FROM \`payload_locked_documents_rels\`;`)
